...
<div id="table" class="content">
    <div class="form">
        ...
        <Form :inline="true" :model="searchForm" class="demo-form-inline">
            <FormItem label="职能名称">
                <Input v-model="searchForm.title" placeholder="职能名称"></Input>
            </FormItem>
        </Form>
    </div>
  ...
  export default {
    ...
    methods: {
        onlineCompetency (val) {
          this.$axios({type: 'post', url: "/profession/issueProfession", data: {data: JSON.stringify({rcnid: val.rcnid})}, fuc: (result) => {
            if (result.code == 1) {
        ...
        editCompetency (val) {
          this.$router.push({
            ...
            query: {rcnid: val.rcnid}
          })
        },
        pulldownCompetency (val) {
          this.$Modal.confirm({
            content: '点击确定，立即下架【' + val.name + '】职能,下架之后将不能对此进行任何操作。',
            title: '提示',
            onCancel: () => {
              console.log('cancle')
            },
            onOk: () => {
              this.$axios({type: 'post', url: '/profession/closeProfession', data: {data: JSON.stringify({rcnid: val.rcnid})}, fuc: (result) => {
                if (result.code == 1) {
        ...
        getTableData() {
          this.$axios({type: 'post', url: "/profession/queryProfessionList", data: {_start: this.$start, _limit: this.$limit, data: JSON.stringify(this.searchForm)}, fuc: (result) => {
            console.log(result)
